...
import 'package:sawa/features/settings/presentation/screens/settings_screen.dart';
...
void main() {
  testWidgets(
    'SettingsScreen should render user summary card and settings sections',
    (tester) async {
        ...
        ProviderScope(
          overrides: [
            ...
            activeHomeIdProvider.overrideWith(
              (ref) => Stream.value('home-123'),
            ),
            userHomesProvider.overrideWith(
              (ref) => Stream.value(<HomeModel>[]),
            ),
            cachedActiveHomeIdProvider.overrideWithValue(null),
            cachedUserHomesProvider.overrideWithValue(<HomeModel>[]),
            cachedActiveHomeProvider.overrideWithValue(null),
          ],
      ...
      expect(
        find.text(localizations.translate('account_area')),
        findsOneWidget,
      );
      ...
      expect(
        find.text(localizations.translate('home_organization')),
        findsOneWidget,
      );
      expect(
        find.text(localizations.translate('app_preferences')),
        findsOneWidget,
      );
    },
  );
