...
import 'package:beity/features/inventory/domain/usecases/update_inventory_quantity_usecase.dart';
...
void main() {
  ...
  group('UpdateInventoryQuantityUseCase', () {
    ...
    test('should return null and delete item when quantity is <= 0', () async {
      when(() => mockRepository.getInventoryItemById(itemId: any(named: 'itemId')))
          .thenAnswer((_) async => tItem);
      when(() => mockRepository.deleteInventoryItem(itemId: any(named: 'itemId')))
          .thenAnswer((_) async => {});
      ...
      final result = await useCase(itemId: tItemId, homeId: tHomeId, newQuantity: 0);

      expect(result, isNull);
      verify(() => mockRepository.getInventoryItemById(itemId: tItemId)).called(1);
      verify(() => mockRepository.deleteInventoryItem(itemId: tItemId)).called(1);
    ...
    test('should update item quantity and write transaction when quantity is > 0', () async {
      when(() => mockRepository.getInventoryItemById(itemId: any(named: 'itemId')))
          .thenAnswer((_) async => tItem);
      ...
      final result = await useCase(itemId: tItemId, homeId: tHomeId, newQuantity: 3.0);

      expect(result, tUpdatedItem);
      verify(() => mockRepository.getInventoryItemById(itemId: tItemId)).called(1);
      verify(() => mockRepository.updateInventoryItem(itemId: tItemId, quantity: 3.0)).called(1);
      verify(() => mockRepository.createTransaction(
            inventoryItemId: tItemId,
            homeId: tHomeId,
            previousQuantity: 5.0,
