...
void main() {
  ...
  group('AddInventoryItemUseCase', () {
    ...
    test(
      'should update quantity and create transaction when duplicate is found',
      () async {
        when(
          () => mockRepository.findDuplicateItem(
        ...
        verify(
          () => mockRepository.findDuplicateItem(homeId: tHomeId, name: tName),
        ).called(1);
        verify(
          () => mockRepository.updateInventoryItem(
            itemId: tExistingItem.id,
            quantity: 3.0,
          ),
        ).called(1);
        verify(
    ...
    test(
      'should create new item and transaction when duplicate is not found',
      () async {
        when(
          () => mockRepository.findDuplicateItem(
            homeId: any(named: 'homeId'),
        ...
        when(
          () => mockRepository.createInventoryItem(
            homeId: any(named: 'homeId'),
            name: any(named: 'name'),
            quantity: any(named: 'quantity'),
            unitId: any(named: 'unitId'),
