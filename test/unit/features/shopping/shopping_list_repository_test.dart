...
import 'package:beity/features/shopping_lists/data/repositories/supabase_shopping_list_repository.dart';
...
void main() {
  ...
  setUp(() {
    ...
    registerFallbackValue(Uri.parse('https://example.com')); // registered in case it's needed
  ...
  group('SupabaseShoppingListRepository', () {
    test('getShoppingLists should build query and return list of models', () async {
      ...
      when(() => mockFilterBuilder.eq('home_id', 'home-123')).thenAnswer((_) => mockFilterBuilder);
      when(() => mockFilterBuilder.filter('deleted_at', 'is', null)).thenAnswer((_) => mockFilterBuilder);
      
      final fakeTransform = FakePostgrestTransformBuilder<List<Map<String, dynamic>>>(tResponse);
      when(() => mockFilterBuilder.order('updated_at', ascending: false))
    ...
    test('getShoppingListById should return model when found', () async {
      ...
      when(() => mockFilterBuilder.eq('id', 'list-123')).thenAnswer((_) => mockFilterBuilder);
      when(() => mockFilterBuilder.filter('deleted_at', 'is', null)).thenAnswer((_) => mockFilterBuilder);
      
      final fakeTransform = FakePostgrestTransformBuilder<Map<String, dynamic>?>(tResponse);
