...
AiRecipeIngredient testMatchIngredientLocal(
  ...
  if (currentMatch != null) {
    return ing.copyWith(
      ...
      reason: languageCode == 'ar'
          ? 'موجود في القائمة الحالية'
          : 'Already in current list',
...
void main() {
  group('LocalIngredientMatcher & LocalFoodKeyMapper Tests', () {
    test('Scenario 1: AI returns tomatoes and mapper translates to tomato', () {
      ...
      final rawKeyResult = LocalFoodKeyMapper.normalizeFoodKey(
        'fresh_red_tomatoes',
      );
    ...
    test(
      'Scenario 2: AI returns unknown food key but name is "بندورة" which is in inventory => available',
      () {
    ...
    test(
      'Scenario 3: Item in completed/purchased shopping lists => missing',
      () {
        final context = LocalIngredientContextBuilder.build(
          inventoryItems: [],
          currentListItems: [
          ...
          otherListsItems: {
            'قائمة قديمة': [
              MockShoppingItem(
                ...
                quantity: 1,
                isPurchased: true,
              ), // Purchased in other list
            ],
          },
        );

        const ingTomato = AiRecipeIngredient(name: 'طماطم');
        final matchedTomato = testMatchIngredientLocal(
          ingTomato,
          context,
          'ar',
        );
    ...
    test(
      'Scenario 4: Item exists in both Inventory and Current List => Inventory priority (available)',
      () {
        final context = LocalIngredientContextBuilder.build(
          inventoryItems: [MockInventoryItem(name: 'بندورة', quantity: 2)],
          currentListItems: [MockShoppingItem(name: 'طماطم', quantity: 3)],
    ...
    test(
      'Scenario 5: Item exists in Current List and another list => Current List priority (inCurrentList)',
      () {
        final context = LocalIngredientContextBuilder.build(
          inventoryItems: [],
          currentListItems: [MockShoppingItem(name: 'دجاج', quantity: 1)],
          currentListName: 'عشاء اليوم',
          otherListsItems: {
            'مقاضي الأسبوع': [MockShoppingItem(name: 'دجاج', quantity: 2)],
          },
        );
    ...
    test(
      'Scenario 6: Legacy AI returns status: missing, but locally found in Inventory => available',
      () {
        final context = LocalIngredientContextBuilder.build(
          inventoryItems: [MockInventoryItem(name: 'بندورة', quantity: 4)],
          currentListItems: [],
          currentListName: 'قائمتي',
          otherListsItems: {},
        );
    ...
    test(
      'Scenario 7: Low confidence match (confidence < 0.75) => unknown status and custom foodKey',
      () {
        // "صلصة حارة جداً" is not in synonyms map and has low similarity to tomato or chicken
        final matchResult = LocalFoodKeyMapper.match(
          'صلصة حارة جداً جداً جداً',
        );
    ...
    test(
      'Scenario 8: Present but insufficient inventory quantity (e.g. inventory = 3, recipe = 5) => missing with details',
      () {
        final context = LocalIngredientContextBuilder.build(
          inventoryItems: [
            MockInventoryItem(name: 'بندورة', quantity: 3, unit: 'حبة'),
        ...
        const ing = AiRecipeIngredient(name: 'بندورة', quantity: 5);

        final matched = testMatchIngredientLocal(ing, context, 'ar');
        expect(matched.status, equals(IngredientStatus.missing));
        expect(
          matched.reason,
          contains('متوفر بالمخزن (3) ولكن ناقص (2 ناقصة)'),
        );
      },
    );
  });
