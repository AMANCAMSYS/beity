...
void main() {
  testWidgets(
    'ErrorFormatter should format exceptions correctly with localized messages',
    (tester) async {
          ...
          child: MaterialApp(
            home: Builder(
              builder: (context) {
                ...
                final networkMsg = ef.ErrorFormatter.format(
                ...
                expect(
                  postgrestMsg,
                  localizations.translate('db_duplicate_error'),
                );
                ...
                expect(
                  postgrestMsg2,
                  localizations.translate('db_relation_error'),
                );

                // 4. Test AuthException
                final authMsg = ef.ErrorFormatter.format(
                  const AuthException(message: 'Invalid login credentials'),
                  context,
                );
                expect(authMsg, localizations.translate('invalid_credentials'));

                // 5. Test generic unexpected exceptions
                final genericMsg = ef.ErrorFormatter.format(
                  Exception('Some random error'),
                  context,
                );
                expect(
                  genericMsg,
                  localizations.translate('unexpected_error_retry'),
                );
