      ...
      for (final keyword in forbiddenKeywords) {
        for (final entry in json.entries) {
          expect(
            ...
            isFalse,
            reason:
                'Request key "${entry.key}" contains forbidden keyword "$keyword"',
          );
          if (entry.value is String) {
            expect(
              (entry.value as String).toLowerCase().contains(keyword),
              isFalse,
              reason:
                  'Request value for "${entry.key}" contains forbidden keyword "$keyword"',
            );
          }
        }
      }
