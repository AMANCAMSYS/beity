...
import 'package:beity/features/expenses/presentation/providers/expense_providers.dart';
import 'package:beity/features/expenses/presentation/screens/expense_summary_screen.dart';
import 'package:beity/core/localization/app_localizations.dart';
import 'package:beity/features/categories/domain/entities/category.dart';
import 'package:beity/features/categories/data/models/category_model.dart';
import 'package:beity/features/categories/presentation/providers/categories_provider.dart';
...
void main() {
  ...
  setUp(() {
    ...
    // Default stubs
    when(() => mockRepository.isInitialSyncCompleted(homeId: any(named: 'homeId')))
        .thenAnswer((_) async => true);
    
    when(() => mockSyncCoordinator.syncAll(
      any(),
  ...
  Widget buildScreen({
    ...
    return ProviderScope(
      overrides: [
        ...
        categoriesByTypeProvider((homeId: homeId, type: 'expense')).overrideWith(
          (ref) => Future.value([
            const CategoryModel(id: 'cat-food', homeId: 'home-123', name: 'cat-food', type: CategoryType.expense),
            const CategoryModel(id: 'cat-transport', homeId: 'home-123', name: 'cat-transport', type: CategoryType.expense),
          ]),
        ),
        homeMembersProvider(homeId).overrideWith(
          (ref) => Stream.value([
            HomeMemberModel(id: 'mem-1', homeId: 'home-123', userId: 'user-1', role: 'member', userName: 'user-1', joinedAt: DateTime(2026, 5, 27)),
            HomeMemberModel(id: 'mem-2', homeId: 'home-123', userId: 'user-2', role: 'member', userName: 'user-2', joinedAt: DateTime(2026, 5, 27)),
          ]),
        ),
  ...
  group('ExpenseSummaryScreen - Local-First Reactive Summary', () {
    ...
    testWidgets('1. Opening ExpenseSummaryScreen does not call Supabase', (tester) async {
    ...
    testWidgets('2. Changing date or filters does not call Supabase', (tester) async {
      await tester.pumpWidget(buildScreen(expenses: testExpenses));
      await tester.pumpAndSettle();
    ...
    testWidgets('3. Adding local expense updates the summary reactively', (tester) async {
    ...
    testWidgets('4. Pull to Refresh / Error Retry triggers Delta Sync for expenses only', (tester) async {
      // Simulate repository error to show retry button
      await tester.pumpWidget(buildScreen(
        expensesStream: Stream.error(Exception('Cache error')),
      ));
      await tester.pumpAndSettle();
      ...
      verify(() => mockSyncCoordinator.syncAll(
        'home-123',
        force: true,
    ...
    testWidgets('5. Empty cache before initial sync shows Preparing/Loading state instead of empty state', (tester) async {
      await tester.pumpWidget(buildScreen(
        expenses: [],
    ...
    test('6. endDate includes full last day (inclusive 23:59:59)', () async {
      final container = ProviderContainer(
        overrides: [
          expensesProvider('home-123').overrideWith((ref) => Stream.value([
            Expense(
              id: 'exp-late',
              homeId: 'home-123',
              amount: 1000,
      ...
      // Wait for the stream to emit its first data event
      await container.read(expensesProvider('home-123').future);
      
      // Read the summary with endDate set to 2026-05-15 (but no time specified, i.e. 00:00:00)
      final summaryAsync = container.read(expenseSummaryProvider((
        homeId: 'home-123',
