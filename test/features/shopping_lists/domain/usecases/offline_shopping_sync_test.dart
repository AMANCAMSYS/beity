...
void main() {
  group('Offline Shopping Sync', () {
    ...
    test('offline update does not crash', () async {
      ...
      expect(
        () async =>
            syncService.updateItemOffline('item_1', {'is_completed': true}),
        returnsNormally,
      );
    ...
    test('sync uses correct homeId', () async {
      ...
      await syncService.createItemOffline({
        'id': 'temp_1',
        'name': 'Apples',
        'home_id': 'home1',
      });
  ...
  Future<void> sync() async {
    for (var op in _offlineQueue) {
      if (op['action'] == 'create') {
        ...
        serverItem['id'] =
            'server_${data['id']}'; // Simulate server ID generation
