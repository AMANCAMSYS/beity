...
void main() {
  group('SyncQueueUseCase Tests', () {
    ...
    test(
      'Test 1: Temp ID mapping sequence (Add item -> Update/Toggle/Delete)',
      () async {
        ...
        expect(
          queueRepository.entries.isEmpty,
          true,
          reason: 'Successful entries should be deleted from queue',
        );
    ...
    test(
      'Test 2: Failure handling when Supabase fails to return real ID (maybeSingle returns null)',
      () async {
        ...
        final syncUseCase = SyncQueueUseCase(
          ...
          executeAction: (entry) async {
            ...
            if (entry.actionType == ActionType.updateItem) {
              ...
              throw Exception(
                'PostgrestException: 404 Not Found - Entity not found',
              );
            }
            return null;
          },
    ...
    test(
      'Test 3: Compaction - Create + Delete offline should discard both',
      () async {
    ...
    test(
      'Test 4: Compaction - Update + Delete for existing item should compact to delete only',
      () async {
        final now = DateTime.now();
        final updateEntry = QueueEntry(
          id: 1,
    ...
    test(
      'Test 5: Compaction - Create + Update should merge into single Create',
      () async {
        final now = DateTime.now();
        final addEntry = QueueEntry(
          id: 1,
    ...
    test('Test 6b: Locking is scoped per home', () async {
      connectivityRepository.status = DeviceSyncStatus.online;
    ...
    test(
      'Test 6c: Shared lock coordinates separate use case instances',
      () async {
        connectivityRepository.status = DeviceSyncStatus.online;
        ...
        final sharedLock = SyncQueueLock();
        final firstUseCase = SyncQueueUseCase(
          queueRepository: queueRepository,
          connectivityRepository: connectivityRepository,
          syncLock: sharedLock,
          executeAction: (e) async {
        ...
        final secondUseCase = SyncQueueUseCase(
          queueRepository: queueRepository,
          connectivityRepository: connectivityRepository,
          syncLock: sharedLock,
        ...
        final firstRun = firstUseCase.execute('home-1');
        final secondResult = await secondUseCase.execute('home-1');
        final firstResult = await firstRun;
    ...
    test(
      'Test 7: Crash Recovery - Resets syncing status to pending before run',
      () async {
        final entry = QueueEntry(
          id: 1,
          actionType: ActionType.addItem,
    ...
    test(
      'Test 7b: Crash Recovery - Resets user-scope syncing status',
      () async {
        final entry = QueueEntry(
          id: 1,
        ...
        final result = await syncUseCase.executeUserScope('user-1');

        expect(result.successCount, 1);
        expect(queueRepository.entries, isEmpty);
      },
    );
    ...
    test('Test 8: Smart Retry - Permanent vs Temporary Errors', () async {
      ...
      final syncUseCase = SyncQueueUseCase(
        ...
        executeAction: (entry) async {
          if (entry.entityId == 'item-1') {
            throw Exception(
              'PostgrestException: 400 Bad Request - Column not found',
            );
          } else {
      ...
      final recoveredPerm = queueRepository.entries.firstWhere(
        (e) => e.id == 1,
      );
      final recoveredTemp = queueRepository.entries.firstWhere(
        (e) => e.id == 2,
      );
