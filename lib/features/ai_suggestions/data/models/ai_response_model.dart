...
class AiResponseModel {
  static AiResponse fromJson(Map<String, dynamic> json) {
    ...
    switch (type) {
      ...
      case 'shopping_suggestions':
        final suggestions = _parseList(
          json['suggestions'] ?? json['sug'],
          _parseSuggestion,
        );
      ...
      case 'meal_suggestions':
        ...
        final sumVal =
            json['sum'] as String? ?? json['summary'] as String? ?? '';
        final meals = _parseList(mealsVal, _parseMeal);
        return AiMealSuggestionsResponse(summary: sumVal, meals: meals);

      case 'recipe_ingredients':
        final mealVal = json['meal'] ?? json['m'];
        final ingredientsVal = json['ingredients'] ?? json['ing'] ?? [];
        final optionalIngredientsVal =
            json['optionalIngredients'] ?? json['opt_ing'] ?? [];
        final stepsVal =
            json['cookingStepsPreview'] ?? json['st'] ?? json['steps'] ?? [];
        ...
        return AiRecipeIngredientsResponse(
          ...
          optionalIngredients: _parseList(
            optionalIngredientsVal,
            (j) => _parseIngredient(j, isRequired: false),
          ),
          cookingStepsPreview: _parseStringList(stepsVal),
          shoppingSummary: _parseShoppingSummary(
            json['shoppingSummary'] as Map<String, dynamic>?,
          ),
        );
  ...
  static List<T> _parseList<T>(
    ...
    if (val is! List) return [];
    return val.whereType<Map<String, dynamic>>().map(parser).toList();
  }
  ...
  static AiMeal _parseMeal(Map<String, dynamic> json) {
    ...
    final description =
        _safeStr(json['d']) ?? _safeStr(json['description']) ?? '';
    final difficulty =
        _safeStr(json['df']) ?? _safeStr(json['difficulty']) ?? '';
    final time = _safeInt(
      json['t'],
      _safeInt(json['time'], _safeInt(json['estimatedTimeMinutes'], 30)),
    );
  ...
  static AiPantryMeal _parsePantryMeal(Map<String, dynamic> json) {
    return AiPantryMeal(
      ...
      estimatedTimeMinutes: _safeInt(
        json['t'],
        _safeInt(json['time'], _safeInt(json['estimatedTimeMinutes'], 30)),
      ),
      servings: _safeInt(json['srv'], _safeInt(json['servings'], 4)),
      cuisine: _safeStr(json['cuisine']) ?? '',
      availableIngredients: _parseStringList(
        json['availableIngredients'] ?? json['av_ing'],
      ),
      missingIngredients: _parseStringList(
        json['missingIngredients'] ?? json['mis_ing'],
      ),
  ...
  static AiRecipeIngredient _parseIngredient(
    ...
    final foodKey =
        _safeStr(json['k']) ??
        _safeStr(json['food_key']) ??
        _safeStr(json['foodKey']);
  ...
  static AiPlanDay _parsePlanDay(Map<String, dynamic> json) {
    ...
    return AiPlanDay(day: json['day'] as String? ?? '', meals: meals);
  }
