...
class _AiSuggestionsScreenState extends ConsumerState<AiSuggestionsScreen> {
  ...
  Future<(List<String>, List<String>, Map<String, String>)> _gatherLocalContext(String localeCode) async {
    final List<String> formattedShoppingItems = [];
    List<InventoryItemModel> inventoryItems = [];
    
    try {
      inventoryItems = ref.read(inventoryItemsProvider(widget.homeId)).valueOrNull ?? [];
    } catch (_) {}
    
    List<ShoppingListModel> lists = [];
    try {
      final listsAsync = ref.read(shoppingListsProvider(widget.homeId));
    ...
    for (final list in uncompletedLists) {
      ...
      final listSuffix = isCurrentList
          ? (localeCode == 'ar' ? 'القائمة الحالية: ${list.name}' : 'current list: ${list.name}')
          : (localeCode == 'ar' ? 'قائمة أخرى: ${list.name}' : 'other list: ${list.name}');
    ...
    final inventorySuffix = localeCode == 'ar' ? 'المخزن' : 'inventory';
    ...
    final userTerms = LocalIngredientContextBuilder.buildUserTerms(localContext);
  ...
  void _showRecipeIngredientsInstantly(String mealName, String description, List<String> mainIngredients, int estimatedTimeMinutes, int servings) {
    ...
    ActionDebouncer.execute(() async {
      ...
      ref.read(aiAssistantProvider.notifier).getRecipeIngredients(
        mealName: mealName,
        language: localeCode,
        homeId: widget.homeId,
        listId: widget.listId,
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text(context.translate('added_x_items_successfully', arguments: {'count': selectedIndices.length.toString()})),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        actions: [
          if (state is AiAssistantResult && (state.response is AiRecipeIngredientsResponse || state.response is AiShoppingSuggestionsResponse))
            PopupMenuButton<String>(
              ...
              itemBuilder: (context) => [
                PopupMenuItem(
                  value: 'select_all',
                  child: Text(
                    state.response is AiRecipeIngredientsResponse || state.response is AiShoppingSuggestionsResponse
                        ? context.translate('select_all_missing')
                        : context.translate('select_all'),
                  ),
                ),
                PopupMenuItem(value: 'deselect_all', child: Text(context.translate('deselect_all'))),
              ],
            ),
        ],
      ),
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text(context.translate('ai_thinking'), style: const TextStyle(fontSize: 15)),
            ],
          ),
        ),
      );
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                Text(context.translate(state.message), textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, color: AppColors.error)),
                const SizedBox(height: 16),
                ElevatedButton.icon(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              const CircularProgressIndicator(color: AppColors.primary),
              const SizedBox(height: 16),
              Text(context.translate('adding_items'), style: const TextStyle(fontSize: 15)),
            ],
          ),
        ),
      );
            ...
            return Container(
              ...
              child: CheckboxListTile(
                ...
                title: Row(
                  children: [
                    Expanded(
                      child: Text(s.displayName ?? s.name, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 15)),
                    ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text(context.translate(response.message), textAlign: TextAlign.center, style: const TextStyle(fontSize: 15)),
                ],
              ),
            ),
          ),
      ...
      case AiErrorResponse():
        return Center(
          child: SingleChildScrollView(
            child: Padding(
              padding: const EdgeInsets.all(32),
              child: Text(context.translate(response.message), textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, color: AppColors.error)),
            ),
          ),
        );
    }
        ...
        child: Row(
          children: [
            ...
            Expanded(
              flex: 2,
              child: BeityButton(
                text: context.translate('add_selected', arguments: {'count': selectedCount.toString()}),
  ...
  String _getHintText() {
    switch (_selectedMode) {
      case AiMode.shoppingSuggestions: return context.translate('ai_hint_shoppingSuggestions');
      case AiMode.whatToCook: return context.translate('ai_hint_whatToCook');
      case AiMode.recipeIngredients: return context.translate('ai_hint_recipeIngredients');
      case AiMode.cookByVegetables: return context.translate('ai_hint_cookByVegetables');
      case AiMode.cookBySpices: return context.translate('ai_hint_cookBySpices');
      case AiMode.cookByAvailable: return context.translate('ai_hint_cookByAvailable');
      case AiMode.budgetMeals: return context.translate('ai_hint_budgetMeals');
      case AiMode.healthyMeals: return context.translate('ai_hint_healthyMeals');
      case AiMode.quickMeals: return context.translate('ai_hint_quickMeals');
      case AiMode.kidsMeals: return context.translate('ai_hint_kidsMeals');
      case AiMode.guestMeals: return context.translate('ai_hint_guestMeals');
      case AiMode.weeklyMealPlan: return context.translate('ai_hint_weeklyMealPlan');
      case AiMode.ramadanList: return context.translate('ai_hint_ramadanList');
      case AiMode.travelList: return context.translate('ai_hint_travelList');
      case AiMode.cleaningList: return context.translate('ai_hint_cleaningList');
    }
  }
}
