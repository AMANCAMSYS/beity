...
final expensesProvider = StreamProvider.autoDispose
    .family<List<Expense>, String>((ref, homeId) {
      if (homeId.isEmpty) {
        return Stream.value([]);
      }
...
final expenseTotalProvider =
    FutureProvider.family<
      int,
      ({String homeId, DateTime? startDate, DateTime? endDate})
    >((ref, params) async {
      final repository = ref.watch(expenseRepositoryProvider);
...
final initialSyncCompletedExpensesProvider = StreamProvider.autoDispose
    .family<bool, String>((ref, homeId) async* {
      final repository = ref.watch(expenseRepositoryProvider);
...
final expenseSummaryProvider = Provider.autoDispose
    .family<
        ...
        data: (expenses) {
          ...
          for (final expense in filteredExpenses) {
            ...
            memberBreakdown[expense.paidBy] =
                (memberBreakdown[expense.paidBy] ?? 0) +
